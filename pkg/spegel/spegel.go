...
func (c *Config) Start(ctx context.Context, nodeConfig *config.Node, criReadyChan <-chan struct{}) error {
	...
	regSvr := reg.Handler(logr.Discard())
	...
	go func() {
		<-criReadyChan
		state.Track(ctx, ociStore, router, state.WithResolveLatestTag(resolveLatestTag), state.WithInterval(time.Minute))
	}()
	...
	mRouter.PathPrefix("/v2").Handler(regSvr)
