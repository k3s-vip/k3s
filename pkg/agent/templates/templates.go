...
var templateFuncs = template.FuncMap{
	"deschemify": func(s string) string {
		...
		for _, scheme := range []string{"unix:", "npipe:"} {
			...
			return strings.TrimPrefix(s[len(scheme):], "//")
		}
		return s
	},
}
...
version = 3
imports = [{{ printf "%q" (printf "%s.d/*.toml" .NodeConfig.Containerd.Config) }}]
...
[plugins.'io.containerd.snapshotter.v1.stargz']
  cri_keychain_image_service_path = {{ deschemify . | printf "%q" }}
